...
2. Work in your git worktree on branch `wip/<your id>` created from `origin/main`. Touch only the paths you own (CONTRACTS §1). `ios/SkiPassCore/Package.swift` and `SkiPassModels` are fixed; if you need a change, stop and report it.
