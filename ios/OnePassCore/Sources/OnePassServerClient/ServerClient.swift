...
/// Configuration for `ServerClient` (values come from Info.plist keys `OnePassServerURL` /
/// `OnePassAppToken` and the App Group key `rc.appUserID`, CONTRACTS §2 and §4).
public struct ServerClientConfiguration: Sendable {
    /// Worker base URL, e.g. `https://onepass-server.example.workers.dev`.
    public var baseURL: URL
    /// Sent as `X-OnePass-App-Token`.
    public var appToken: String
    /// Returns the RevenueCat app user ID, sent as `X-OnePass-User`; nil or empty = not available yet.
...
/// Client for the OnePass server HTTP API (CONTRACTS §5).
public struct ServerClient: CandidateJudging, UsageReporting {
    ...
    private func send<Body: Encodable>(path: String, method: String, body: Body?) async throws -> (Data, Int) {
        ...
        request.setValue(configuration.appToken, forHTTPHeaderField: "X-OnePass-App-Token")
        request.setValue(user, forHTTPHeaderField: "X-OnePass-User")
        if let body {
