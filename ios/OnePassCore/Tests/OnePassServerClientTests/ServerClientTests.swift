...
final class ServerClientTests: XCTestCase {
    ...
    func testJudgeSendsContractRequest() async throws {
        ...
        XCTAssertEqual(sent.request.url?.absoluteString, "https://onepass-server.example.workers.dev/v1/judge")
        XCTAssertEqual(sent.request.value(forHTTPHeaderField: "Content-Type"), "application/json")
        XCTAssertEqual(sent.request.value(forHTTPHeaderField: "X-OnePass-App-Token"), "test-app-token")
        XCTAssertEqual(sent.request.value(forHTTPHeaderField: "X-OnePass-User"), user)
